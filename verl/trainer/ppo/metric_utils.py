...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> Dict[str, Any]:
    ...
    metrics = {
        ...
        "critic/coverage/mean": batch.non_tensor_batch['coverage'].mean(),
        "critic/coverage/max": batch.non_tensor_batch['coverage'].max(),
        "critic/coverage/min": batch.non_tensor_batch['coverage'].min(),

        "critic/accuracy/mean": batch.non_tensor_batch['accuracy'].mean(),
        "critic/accuracy/max": batch.non_tensor_batch['accuracy'].max(),
        "critic/accuracy/min": batch.non_tensor_batch['accuracy'].min(),

        "critic/writing_reward/mean": batch.non_tensor_batch['writing_reward'].mean(),
        "critic/writing_reward/max": batch.non_tensor_batch['writing_reward'].max(),
        "critic/writing_reward/min": batch.non_tensor_batch['writing_reward'].min(),
    ...
    # 所有要计算统计量的指标
    all_keys = [
        'lexical_bleu', 'semantic_similarity', 'contextual_relevance',
        'paper_coverage', 'graph_coverage', 'kp_faithfulness_graph',
        'kp_consistency_paper', 'entailment_faithfulness', 'entailment_consistency', "reference_recall", 
        # 新增 LLM 指标
    ...
    # 循环计算统计量
    for reward_name in all_keys:
        log_prefix = f"critic/{reward_name}"
        metrics[log_prefix + "/mean"] = batch.non_tensor_batch[reward_name].mean()
        metrics[log_prefix + "/max"] = batch.non_tensor_batch[reward_name].max()
        metrics[log_prefix + "/min"] = batch.non_tensor_batch[reward_name].min()
